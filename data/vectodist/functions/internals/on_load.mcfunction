# GITHUB PAGE: -missing-
#[STORAGE] vtd vec set value [0.0d, 0.0d, 0.0d]

scoreboard objectives add vtd dummy
scoreboard players add dist vtd 0

scoreboard players set 1 vtd 1
scoreboard players set 10 vtd 10
scoreboard players set 100 vtd 100
scoreboard players set 1000 vtd 1000
scoreboard players set 10000 vtd 10000
scoreboard players set 100000 vtd 100000
scoreboard players set 1000000 vtd 1000000
scoreboard players set 10000000 vtd 10000000
scoreboard players set 100000000 vtd 100000000
scoreboard players set 1000000000 vtd 1000000000

forceload add 0 0